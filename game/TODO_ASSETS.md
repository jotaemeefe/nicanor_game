...
| Ventilador de techo — rotor + eje | `assets/props/ventilador/fan_rotor.png`, `fan_mount.png` | `nuevos_assets/ambient/ceiling_fan_4_frames.png` (cuadro 4) | Cortados por script del cuadro 4 de la hoja, ambos centrados en el buje: el rotor (aspas + buje) gira continuamente, el eje (caño + florón) queda fijo y se dibuja encima. Reemplaza a `fan_a/fan_b` + `fan_frames.tres`, cuyos recortes tenían el ventilador en puntos distintos del lienzo y lo hacían saltar 42 px cada 1,4 s. El `modulate` de la escena baja el brillo del recorte al del ventilador pintado en el maestro. |
