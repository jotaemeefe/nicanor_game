# PrismDefense (Visuals variant) — Technical Design Document

> Variant delta vs `samples/PrismDefense3D/`: the rendering module additionally
> loads three 512px CC0 color maps (ambientCG Rock035/Ground037/Grass001, in
> `public/assets/textures/`) applied to the tiles, path, and a 90x90 terrain
> plane; a runtime-generated gradient sky + fog; one extra point light for the
> crystal's pulsing glow; unlit projectile materials. Loading is async with the
> flat-color look as fallback; textures are tracked for dispose(). Everything
> else — sim, input, UI, audio, instancing, pools — is identical.
